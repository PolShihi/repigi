def float_validation(value: str) -> float:
    '''
    This function checks whether a given value can be converted to a float. \
It attempts to convert the value to a float and raises a ValueError if the conversion fails.
...
def absolute_less_than_one_validation(value: float) -> float:
    '''
    This function checks whether a given value is between -1 and 1 (excluding -1 and 1). \
It raises a ValueError if the value is not within the specified range.
...
def int_validation(value: str) -> int:
    '''
    This function checks whether a given value can be converted to an integer. \
It attempts to convert the value to an integer and raises a ValueError if the conversion fails.
...
def between_0_and_1_validation(value: float) -> float:
    '''
    Validates whether the input value falls within the range (0, 1].

    Parameters:
    value (float): The input value to be validated.

    Returns:
    float: The validated value.

    Raises:
    ValueError: If the value is outside the range (0, 1].
    '''

    if value <= 0 or value > 1:
        raise ValueError("Enter a value between 0 and 1, 0 excluded")
